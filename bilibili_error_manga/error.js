(function (r) {
    var pic_pool = [], o = ["http://activity.hdslb.com/zzjs/cartoon/errorPage-manga-1.png", "http://activity.hdslb.com/zzjs/cartoon/errorPage-manga-2.png", "http://activity.hdslb.com/zzjs/cartoon/errorPage-manga-3.png", "http://activity.hdslb.com/zzjs/cartoon/errorPage-manga-4.png", "http://activity.hdslb.com/zzjs/cartoon/errorPage-manga-5.png", "http://activity.hdslb.com/zzjs/cartoon/errorPage-manga-6.png", "http://activity.hdslb.com/zzjs/cartoon/errorPage-manga-7.png"];
    var t = false;
    r.ajax(
        {
            ...
            success: function (r) {
                ...
                for (var e in o) {
                    pic_pool.push(
                        (o[e].data && o[e].data.img).replace(/^http:/, "")
                    )
                }
                t = true;
            }
    ...
    var e = function (r) {
        ...
        var n = i[d];
        if (n != r) { return n } else { return e(r) }
    };
    var i = function () {
        rec_rp("event", "errorpage_btnback_click");
        if (history.length > 2) { history.back() } else { location.href = document.referrer || "//www.bilibili.com" }
    };
    if (history.length > 2 || document.referrer) { var n = "返回上一页" } else { var n = "返回首页" } var c = r(".error-container");
    ...
    rec_rp("event", "errorpage_pageshow", { pic: g, url: window.location.href, errorType: options.type });
    ...
    if (window.options && options.type == "defaultError") {
        ...
        s.find(".panel");
        r("a.rollback-btn", s).click(function () { i() })
        ...
        switch (parseInt(m)) {
            case 701: (function () {
                ...
                if (window.options && options.data && options.data.url) {
                    ...
                    setInterval(function () {
                        ...
                        r--;
                        if (r < 0) { location.href = options.data.url }
                    }, 1e3)
                }
            })();
                break;
            case 702: (function () {
                ...
                s.append(r('<div class="rollback-btn">' + n + "</div>").click(function () { i() }))
            })();
                break;
            case 703: (function () {
    ...
    r("a.change-img-btn", l).click(function (o) {
        if (r("a.change-img-btn", l).hasClass("off")) { return } var t = r("img", l).attr("src");
        rec_rp("event", "errorPage_btnrefresh_click", { pic: t, url: window.location.href, errorType: options.type });
        r("img", l).attr("src", e(t)).one("load", function () {
            r("a.change-img-btn", l).removeClass("off");
            clearTimeout(p);
            rec_rp("event", "errorpage_pageshow", { pic: r("img", l).attr("src"), url: window.location.href, errorType: options.type })
        });
        r(this).addClass("off");
        p = setTimeout(function () { r("a.change-img-btn", l).removeClass("off") }, 3e3);
        f++;
        if (f == 100) { (new MessageBox).show(r(this), "别刷了，其实一共就" + (pic_pool.length + 1) + "张(笑)", 3e3) } else if (f == 200) { (new MessageBox).show(r(this), "好吧骗你的，其实一共就" + pic_pool.length + "张(笑)", 3e3) }
    });
    r(".error-split").attr("id", "up");
    r(".change-img-btn").attr("href", "#up");
    var h = '<div class="msg-text">' + (
        options && options.data && options.data.message || "") + "</div>";
